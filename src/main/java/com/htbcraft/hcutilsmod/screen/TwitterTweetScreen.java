...
public class TwitterTweetScreen extends Screen {
    ...
    private void dispTweetTextLength() {
        ...
        if (this.tweetEdit1 != null) {
            len += this.tweetEdit1.getText().length();
        }
        if (this.tweetEdit2 != null) {
            len += this.tweetEdit2.getText().length();
        }
        if (this.tweetEdit3 != null) {
            len += this.tweetEdit3.getText().length();
        }
        if (this.tweetEdit4 != null) {
            len += this.tweetEdit4.getText().length();
        }

        this.tweetLength =
                new TranslationTextComponent("hcutilsmod.settings.twitter.body.length").appendString(String.valueOf(len));
    }

    // ツイート本文
    public String getTweetTextBody() {
        return this.tweetEdit1.getText() +
                this.tweetEdit2.getText() +
                this.tweetEdit3.getText() +
                this.tweetEdit4.getText();
    }

    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        this.renderBackground(matrixStack);
        drawString(matrixStack, this.font, this.title, 12, 40, 16777215);
        this.message.func_241865_b(matrixStack, 12, 60, 20, 16777215);
