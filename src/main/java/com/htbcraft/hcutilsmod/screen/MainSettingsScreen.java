...
public class MainSettingsScreen extends SettingsScreen {
    ...
    protected void init() {
        ...
        this.addButton(new Button(getPosX() + (getWidth() - 180) / 2, getPosY() + 80, 180, 20,
            new TranslationTextComponent("hcutilsmod.settings.findspawner.title").appendString("..."),
        ...
        this.addButton(new Button(getPosX() + (getWidth() - 180) / 2, getPosY() + 105, 180, 20,
            new TranslationTextComponent("hcutilsmod.settings.brightness.title").appendString("..."),
            (var1) -> this.getMinecraft().displayGuiScreen(new BrightnessSettingsScreen(this))));

        if (HCCrypt.isSupportOS()) {
            // Twitter設定
            this.addButton(new Button(getPosX() + (getWidth() - 180) / 2, getPosY() + 130, 180, 20,
                new TranslationTextComponent("hcutilsmod.settings.twitter.title").appendString("..."),
                (var1) -> this.getMinecraft().displayGuiScreen(new TwitterSettingsScreen(this))));
        }
