...
public class TwitterDefaultBodyScreen extends SettingsScreen {
    ...
    protected void init() {
        ...
        this.addButton(new Button(getPosX() + (getWidth() - 100) / 2, getPosY() + 140, 100, 20,
            ...
            (var1) -> {
                updateTweetText();
                this.getMinecraft().displayGuiScreen(this.getParent());
            }));
    }
    ...
    private void dispTweetTextLength() {
        ...
        if (this.defTweetEdit1 != null) {
            len += this.defTweetEdit1.getText().length();
        }
        if (this.defTweetEdit2 != null) {
            len += this.defTweetEdit2.getText().length();
        }
        if (this.defTweetEdit3 != null) {
            len += this.defTweetEdit3.getText().length();
        }
        if (this.defTweetEdit4 != null) {
            len += this.defTweetEdit4.getText().length();
        }

        this.tweetBodyDef =
                new TranslationTextComponent("hcutilsmod.settings.twitter.body.default.length").appendString(String.valueOf(len));
    }

    // デフォルト本文の保存
    private void updateTweetText() {
        HCSettings.getInstance().twitterText1 = this.defTweetEdit1.getText();
        HCSettings.getInstance().twitterText2 = this.defTweetEdit2.getText();
        HCSettings.getInstance().twitterText3 = this.defTweetEdit3.getText();
        HCSettings.getInstance().twitterText4 = this.defTweetEdit4.getText();
        LOGGER.info(this.defTweetEdit1.getText() + this.defTweetEdit2.getText() + this.defTweetEdit3.getText() + this.defTweetEdit4.getText());
    }
