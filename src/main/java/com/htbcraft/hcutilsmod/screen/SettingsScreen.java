...
public class SettingsScreen extends Screen {
    ...
    public void init(Minecraft p_init_1_, int p_init_2_, int p_init_3_) {
        ...
        win_x = (p_init_2_ - WINDOW_WIDTH) / 2;
        win_y = (p_init_3_ - WINDOW_HEIGHT) / 2;
    }
    ...
    public void render(int p_230430_2_, int p_230430_3_, float p_230430_4_) {
        ...
        renderWindow(win_x, win_y, WINDOW_WIDTH, WINDOW_HEIGHT, this.title, BACKGROUND_TEXTURE);
        super.render(p_230430_2_, p_230430_3_, p_230430_4_);
    }

    // このメソッドはライブラリ化したい
    public static void renderWindow(int x, int y, int width, int height, ITextComponent title, ResourceLocation background) {
        if (x < 0) {
            x = 0;
        }
        ...
        if (mc.mainWindow.getScaledWidth() < width) {
            width = mc.mainWindow.getScaledWidth();
        }

        if (mc.mainWindow.getScaledHeight() < height) {
        ...
        AbstractGui.blit(x + 10, y + 10, 0, 0, width - 20, height - 20, 16, 16);
        ...
        // ウィンドウ枠の4隅
        AbstractGui.blit(x, y, 0, 0, 20, 30, 256, 256);
        AbstractGui.blit(x + width - 20, y, 252 - 20, 0, 20, 30, 256, 256);
        AbstractGui.blit(x, y + height - 20, 0, 140 - 20, 20, 20, 256, 256);
        AbstractGui.blit(x + width - 20, y + height - 20, 252 - 20, 140 - 20, 20, 20, 256, 256);
        ...
        for (int i = 0; i < count; i++) {
            ...
            AbstractGui.blit(x1, y1, texture_x, texture_y, texture_width, texture_height, 256, 256);
        }
        ...
        // タイトルの描画
        mc.fontRenderer.drawString(title.getString(), (float)x + 8, (float)y + 6, 4210752);
    }
}
